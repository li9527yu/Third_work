...
class MyFlanT5(nn.Module):
    ...
    def forward(
        ...
        pooled_img_feat = input_hidden_states.mean(dim=1)   # [B, 768]
        ...
        fused_logits = gate_text * outputs.logits + gate_img * multi_outputs.logits  # [B, L_dec, V]
        main_loss = None
    ...
    def generate(
        ...
        gate_img = aim_probs[:, emo_rel_idx].view(-1,1,1)         # [B,1,1]
        gate_text = 1.0 - gate_img                                # [B,1,1]
        ...
        for step_logits_text, step_logits_img in zip(outputs.scores, multi_outputs.scores):
            ...
            # step_logits_img:  [B,V]
            fused_logits = gate_text.squeeze(2).squeeze(1)[:,None] * step_logits_text + \
                        gate_img.squeeze(2).squeeze(1)[:,None]  * step_logits_img
