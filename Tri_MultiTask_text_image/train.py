...
def post_dataloader(batch):
    ...
    input_ids,input_multi_ids,input_imgemo_ids,\
    input_attention_mask,input_multi_attention_mask,input_imgemo_attention_mask,\
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,imgemo_labels,senti_label,relation_label=batch
    ...
    input_imgemo_ids = input_imgemo_ids.clone().detach().long().to(device)
    input_imgemo_attention_mask = input_imgemo_attention_mask.clone().detach().float().to(device)

    labels=labels.to(device).long()
    labels_text=labels_text.to(device).long()
    imgemo_labels=imgemo_labels.to(device).long()
    ...
    return input_ids,input_multi_ids,input_imgemo_ids,\
    input_attention_mask,input_multi_attention_mask,input_imgemo_attention_mask,\
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,imgemo_labels,senti_label,relation_label
...
def evaluate(model,test_dataloader,weight, logger):
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        ...
        input_attention_mask,input_multi_attention_mask,input_imgemo_attention_mask,\
        input_hidden_states,input_pooler_outputs,\
        labels_text,labels,imgemo_labels,senti_label,relation_label= post_dataloader(batch)
        with torch.no_grad():
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,weight,logger):
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        ### train
        model.train()
        tot_l,senti_l, senti_text_l,senti_img_l = 0.0, 0.0, 0.0, 0.0
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            ...
            input_hidden_states,input_pooler_outputs,\
            labels_text,labels,imgemo_labels,senti_label,relation_label= post_dataloader(batch)
            total_loss, loss_mm, loss_text,loss_img= model(
                            input_ids=input_ids,
                            input_multi_ids=input_multi_ids,
                            input_imgemo_ids=input_imgemo_ids,
                            attention_mask=input_attention_mask,  
                            input_multi_attention_mask=input_multi_attention_mask,  
                            input_imgemo_attention_mask= input_imgemo_attention_mask,
                            input_hidden_states    = input_hidden_states,
                            labels=labels,
                            imgemo_labels=imgemo_labels,
            ...
            global_step += 1
            tot_l+=total_loss.item()
            senti_l+=loss_mm.item()
            senti_text_l += loss_text.item()
            senti_img_l+=loss_img.item()

        senti_l=senti_l/global_step
        senti_text_l=senti_text_l/global_step
        senti_img_l=senti_img_l/global_step
        tot_l=tot_l/global_step
        logger.info("total_loss:%s",tot_l)
        logger.info("multi_sentiment_loss:%s",senti_l)
        logger.info("text_sentiment_loss:%s",senti_text_l)
        logger.info("img_sentiment_loss:%s",senti_img_l)
        # Dev evaluation
        model.eval()
