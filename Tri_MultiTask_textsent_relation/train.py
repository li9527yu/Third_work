...
def post_dataloader(batch):
    ...
    input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,\
        input_hidden_states,input_pooler_outputs,\
            labels_text,labels,senti_label,relation_semantic,realtion_sentiment=batch
    ...
    relation_semantic=relation_semantic.to(device).long()
    realtion_sentiment=realtion_sentiment.to(device).long()
    ...
    return input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,input_hidden_states,input_pooler_outputs,labels_text,labels,senti_label,relation_semantic,realtion_sentiment
...
def evaluate(model,test_dataloader,weight, logger):
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,\
            input_hidden_states,input_pooler_outputs,\
                labels_text,labels,senti_label,relation_semantic,realtion_sentiment= post_dataloader(batch)
        with torch.no_grad():
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,weight,logger):
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        ### train
        model.train()
        tot_l,senti_l, senti_text_l,relation_l = 0.0, 0.0, 0.0,0.0
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            ...
            input_hidden_states,input_pooler_outputs,\
                labels_text,labels,senti_label,relation_semantic,realtion_sentiment= post_dataloader(batch)
            total_loss, loss_mm, loss_text,loss_rel = model(input_ids=input_ids,
                            input_multi_ids=input_multi_ids,
                            attention_mask=input_attention_mask,  
                            input_multi_attention_mask=input_multi_attention_mask,            
                            input_hidden_states    = input_hidden_states,
                            ...
                            realtion_sentiment=realtion_sentiment,
                            weight=weight
                        )
        ...
        if dev_result['senti_f1'] >= max_f1:
            ...
            logger.info("dev_max_f1: %d",max_f1)
