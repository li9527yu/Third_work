...
def evaluate_single(model, dataset,weight, logger, save_path=None):
    ...
    for i in tqdm(range(len(dataset)), desc="Single-sample Testing"):
        ...
        sample = dataset[i]
        (input_ids,input_multi_ids,input_attention_mask,\
                input_multi_attention_mask,input_hidden_states,input_pooler_outputs,\
                    labels,senti_label,relation_label,subject_label,raw_meta) = sample
        ...
        # === 记录样本信息 ===             "predicted_subject": predicted_subject,
...
def main(args):
    ...
    test_dataset=MyDataset(args,test_data,test_img_data,tokenizer,type='ana')
