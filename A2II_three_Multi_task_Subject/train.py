...
def post_dataloader(batch):
    ...
    input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,\
        input_hidden_states,input_pooler_outputs,\
            labels,senti_label,relation_label,subject_label=batch
    ...
    subject_label=subject_label.to(device).long()
    ...
    return input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,\
        input_hidden_states,input_pooler_outputs,labels,senti_label,relation_label,subject_label
...
def evaluate(model,test_dataloader,weight, logger):
    ...
    for batch in tqdm(test_dataloader, desc="Testing"):
        input_ids,input_multi_ids,input_attention_mask,input_multi_attention_mask,\
            input_hidden_states,input_pooler_outputs,\
                labels,senti_label,relation_label,subject_label= post_dataloader(batch)
        with torch.no_grad():
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,weight,logger):
    ...
    for train_idx in trange(int(args.EPOCHS), desc="Epoch"):
        ...
        ### train
        model.train()
        senti_l,sum_main_loss,sum_subj_loss=0,0,0
        for step,data in enumerate(tqdm(train_dataloader,desc="Iteration")):
            ...
            input_hidden_states,input_pooler_outputs,\
                labels,senti_label,relation_label,subject_label= post_dataloader(batch)
            output_loss,main_loss,loss_subj = model(input_ids=input_ids,
                            input_multi_ids=input_multi_ids,
                            attention_mask=input_attention_mask,  
                            input_multi_attention_mask=input_multi_attention_mask,            
                            input_hidden_states    = input_hidden_states,
            ...
            sum_subj_loss+=loss_subj.item()


        senti_l=senti_l/global_step
        sum_main_loss=sum_main_loss/global_step
        sum_subj_loss=sum_subj_loss/global_step
        logger.info("total_loss: %s, sentiment_loss:%s subject_loss:%s",senti_l,sum_main_loss,sum_subj_loss)
        # wandb.log(
        #     {
        #         "train_loss":senti_l
        #     })
