...
from tool import warmup_linear,parse_sequences,compute_metrics,parse_mm_sequences
...
def post_dataloader(batch):
    ...
    input_ids,input_multi_ids,\
    input_attention_mask,input_multi_attention_mask,\
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,senti_label,relation_label=batch
    ...
    return input_ids,input_multi_ids,\
    input_attention_mask,input_multi_attention_mask,\
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,senti_label,relation_label
...
def evaluate(model,test_dataloader,weight, logger):
    model.eval()
    pred_sequence,text_pred_sequence,senti_labels=[],[],[]
    for batch in tqdm(test_dataloader, desc="Testing"):
        input_ids,input_multi_ids,\
    input_attention_mask,input_multi_attention_mask,\
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,senti_label,relation_label= post_dataloader(batch)
        with torch.no_grad():
            outputs,text_outputs= model.generate(input_ids=input_ids,
            ...
            text_pred_sequence.extend(text_outputs)
            senti_labels.extend(senti_label.detach().cpu().numpy())

    senti_preds=parse_mm_sequences(pred_sequence)
    text_preds=parse_sequences(text_pred_sequence)
...
def train(args,model,train_dataset,val_dataset,optimizer_t5,weight,logger):
    ...
    input_hidden_states,input_pooler_outputs,\
    labels_text,labels,senti_label,relation_label= post_dataloader(batch)
            total_loss, loss_mm, loss_text= model(
                            input_ids=input_ids,
                            input_multi_ids=input_multi_ids,
                            attention_mask=input_attention_mask,  
                            input_multi_attention_mask=input_multi_attention_mask,  
